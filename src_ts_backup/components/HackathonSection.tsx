...
import { motion, AnimatePresence, useMotionValue, useTransform, animate, useScroll, useSpring, useMotionValueEvent } from "framer-motion";
...
// Custom cursor water ripple for timeline area
const TimelineCursor = ({ containerRef }: { containerRef: React.RefObject<HTMLDivElement> }) => {
...
const ScrollProgressLine = ({ containerRef }: { containerRef: React.RefObject<HTMLDivElement> }) => {
  ...
  return (
    <div className="absolute left-5 md:left-1/2 top-0 bottom-0 w-[3px] md:-translate-x-1/2 z-[5]">
      {/* Background line */}
      <div
...
// Extraordinary Timeline Item with alternating layout and scroll reveal
const TimelineItem = ({ phase, index, totalItems }: { phase: { title: string; description: string[] }; index: number; totalItems: number }) => {
  ...
  return (
    <div
      ref={itemRef}
      className={`relative flex items-start gap-0 mb-16 last:mb-0 ${
        isEven ? 'md:flex-row' : 'md:flex-row-reverse'
      } flex-row`}
    >
      {/* Content card - left or right */}
      <motion.div
        className={`flex-1 pl-14 md:pl-0 ${isEven ? 'md:pr-12 md:text-right' : 'md:pl-12 md:text-left'}`}
      ...
      >
        ...
        >
          {/* Glass card */}
          <div
            className="relative rounded-2xl p-6 md:p-8 border border-white/10 overflow-hidden"
            style={{
          ...
          >
            ...
            {/* Phase badge */}
            <motion.div
              className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-display font-bold uppercase tracking-wider mb-4 ${
                isEven ? 'md:ml-auto' : ''
              }`}
              style={{
            ...
            {/* Title */}
            <h4
              className={`text-xl md:text-2xl font-display font-bold mb-4 leading-tight ${isEven ? 'md:text-right' : ''}`}
              style={{
            ...
            <ul className="space-y-3">
              {phase.description.map((point, idx) => (
                <motion.li
                  ...
                  viewport={{ once: true }}
                  transition={{ delay: index * 0.1 + idx * 0.08 + 0.3 }}
                >
                  <motion.span
      ...
      <div className="absolute left-5 md:left-1/2 md:-translate-x-1/2 z-[10] top-6">
        ...
        >
          ...
          <motion.div
            ...
            transition={{ duration: 2, repeat: Infinity, delay: index * 0.15 + 0.5 }}
          />

          {/* Glass dot */}
          <motion.div
            className="w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center border border-white/30 relative overflow-hidden"
            style={{
...
// Parallax water waves background
const ParallaxWaves = ({ containerRef }: { containerRef: React.RefObject<HTMLDivElement> }) => {
  const { scrollYProgress } = useScroll({
    target: containerRef,
  ...
  return (
    <div className="absolute inset-0 pointer-events-none overflow-hidden z-[0]">
        ...
        >
          ...
          >
            <motion.path
              ...
              style={{ filter: `drop-shadow(0 0 20px ${wave.color})` }}
              animate={{ x: [0, -720, 0] }}
              transition={{
          ...
          <svg
            ...
            className="w-[200%] h-full"
            preserveAspectRatio="none"
            style={{ position: 'absolute', left: '-25%', top: '10px' }}
          >
            <motion.path
              d={wave.d}
              fill={wave.color}
              fillOpacity={wave.opacity * 0.6}
              style={{ filter: `drop-shadow(0 0 15px ${wave.color})` }}
              animate={{ x: [0, 720, 0] }}
              transition={{
      ...
      {/* Deep water gradient overlay at bottom */}
      <div
...
const Timeline = ({ type }: { type: 'virtual' | 'physical' }) => {
  ...
  return (
    <div ref={timelineRef} className="py-16 relative cursor-none md:cursor-none" style={{ cursor: 'none' }}>
      {/* Custom cursor */}
      <TimelineCursor containerRef={timelineRef as React.RefObject<HTMLDivElement>} />

      {/* Parallax water wave background */}
      <ParallaxWaves containerRef={timelineRef as React.RefObject<HTMLDivElement>} />

      {/* Background ambient */}
      <div className="absolute inset-0 pointer-events-none">
        <motion.div
          className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[800px] rounded-full blur-3xl"
          style={{ background: 'radial-gradient(circle, hsl(195, 100%, 50% / 0.08), hsl(220, 85%, 40% / 0.05), transparent)' }}
      ...
      >
        <div
          className="inline-block px-10 py-5 rounded-3xl border border-white/10 mb-4"
          style={{
        ...
        <motion.div
          className="w-56 h-[2px] mx-auto mt-4 rounded-full"
          style={{
        ...
        >
          ...
          >
            <motion.div
              className="w-1.5 h-2 rounded-full bg-primary/60"
              animate={{ y: [0, 8, 0] }}
              transition={{ duration: 1.5, repeat: Infinity }}
            />
          </div>
      ...
      {/* Scroll-driven progress line */}
      <ScrollProgressLine containerRef={timelineRef as React.RefObject<HTMLDivElement>} />
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-8">
        ...
        <motion.h3
          className="text-4xl md:text-5xl font-display font-black text-gradient-water"
          animate={{ 
            textShadow: [
...
const HackathonSection = () => {
  ...
  return (
    <section className="relative py-20 md:py-32 overflow-hidden water-bg-effect" id="hackathon">
      {/* Background */}
